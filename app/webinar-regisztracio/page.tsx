...
function WebinarRegForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus('error');
      setErrorMsg(err instanceof Error ? err.message : 'Hiba tortent. Probald ujra!');
    }
  }
  ...
  return (
    <SectionWrapper bg="default">
      <div className="max-w-xl mx-auto">
        {loading && (
          <div className="text-center py-16 font-sans text-brand-muted">Betoltes...</div>
        )}

        {!loading && !webinarId && (
          <div className="text-center py-16 font-sans text-brand-muted">Hianyzo webinar azonosito.</div>
        )}

        {!loading && webinarId && !webinar && (
          <div className="text-center py-16 font-sans text-brand-muted">A webinar nem talalhato.</div>
        )}

        {!loading && webinar && (
          <>
            <div className="mb-8 animate-fade-in">
              <Badge variant="blue" className="mb-3">Webinar</Badge>
              ...
              <div className="flex flex-wrap gap-4 mb-4">
                ...
                {webinar.maxParticipants > 0 && (
                  <span className="flex items-center gap-1.5 text-sm text-brand-muted font-sans">
                    <Users size={14} /> Max. {webinar.maxParticipants} fo
                  </span>
                )}
              </div>
              {webinar.description && (
            ...
            <Card className="animate-fade-in stagger-1">
              {!webinar.registrationOpen ? (
                <div className="text-center py-8 font-sans text-brand-muted">
                  A regisztracio jelenleg zarva van.
                </div>
              ) : status === 'success' ? (
                <div className="text-center py-8">
                  ...
                  <h2 className="font-display text-xl font-bold text-brand-blue mb-2">Sikeres regisztracio!</h2>
                  <p className="font-sans text-brand-muted text-sm">
                    Visszaigazolot kuldtunk a megadott email-cimre.
                  </p>
                </div>
              ) : (
                <>
                  <h2 className="font-display text-xl font-bold text-brand-blue mb-6">Regisztracio</h2>
